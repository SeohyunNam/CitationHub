...
`app.py` in this repository is **not** the CitationHub system (the Next.js / FastAPI website at [citation-hub-website.vercel.app](https://citation-hub-website.vercel.app)).
...
If you want to stand up a dashboard on IDCite quickly without deploying the full CitationHub stack use `app.py` and `requirements.txt` in this repository. Point it at a local Parquet directory (`CITATIONHUB_DATA_DIR`) or a Hugging Face dataset (`HF_REPO_ID`). 
